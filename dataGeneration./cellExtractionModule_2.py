'''
    aim of this code is to read table images and extract cells from it
'''

'''
    this program takes table folder as a input and separates the cc from it
    each cc represents the single cell
    then it creates a dictionary of a dictionary where 1st dictionary contains file name as a key and 2nd dict as a value
    2nd dict contains componant id as a key and cordnate as a value
...
dataPath = basePath +experimentNo+"//table//"
outPath = basePath + experimentNo+"//tableRelatedAll/tableCC//"
dictSave = basePath+experimentNo+"//tableRelatedAll//"
...
def writeImageCC1(OutImage, tempCopy, ii, corList, imgNameFolder):
    ''''
    temporary change
    '''
    ...
    # print("nameImage=",compID+nameImage)#+"_"+str(ii)+".jpg")
    nameImage = compID + "_" + str(corList) + "_.jpg"  # +nameImage
    # print("\n\t nameImage=",nameImage)

    # folderName=nameImage.split(".")[0]

    if not os.path.isdir(OutImage + imgNameFolder):
        os.mkdir(OutImage + imgNameFolder)
        # outfolderPath =folderName+"//"
        # folderName =folderName+"//"

    # cv2.imwrite(OutImage+folderName+nameImage,tempCopy)

    cv2.imwrite(OutImage + imgNameFolder + "//" + nameImage, tempCopy)
...
def displayAndSaveComponents(nm, original, labels, stats, ii, ccCordinate, cc_threshold):
    imageFlag = 0
    ...
    if 1:  # multi>cc_threshold:
        ...
        #print("\n\t original image name=", nm.split("/")[-1])
        imgName = nm.split("/")[-1]
        imgNameFolder = imgName.split(".jpg")[0]
        # print("\n\t imgName=",imgName)
        #writeImageCC1(basePath + "//7//tableRelatedAll//delMe//", tempCopy, ii, [x, y, x + dx, y + dy], imgNameFolder)
        ccCordinate[ii] = [x, y, x + dx, y + dy]
        # input("check")
    return original, ccCordinate
...
def readImages(dataPath):
    ...
    for indx, nm in enumerate(files):
        ...
        try:
            ...
            for ii in np.unique(labels):
                ...
                original, ccCordinate = displayAndSaveComponents(nm, original, labels, stats, ii, ccCordinate,
                                                                 cc_threshold=50)
                idCorDict[nm.split("/")[-1]] = ccCordinate
    ...
    '''
    print("\n\t Dictionary")
    for imgNameKey in idCorDict:
        print "\n\t imgNameKey=",imgNameKey

        #print "\n\t\t val=",idCorDict[imgNameKey]

        for d in idCorDict[imgNameKey].keys():
             print "\n\t\t\t d=",d
             print "\n\t\t\t\t cell=",idCorDict[imgNameKey][d]
    '''
    ...
    np.save(dictSave+"delMe//cellLocation.npy",idCorDict)
...
def extractTableCC(tableCCID, unqElements, labels, nm, img1, phase, scndPhaseImages, folderName1):
    ...
    if phase == 1:
        ...
        cv2.imwrite(cwd + "//" + folderName1 + "//" + imagename + "_phase" + "_table_" + str(phase) + ".jpg",
                    tempImage3)
...
def singleImage(nm, allRectTables, phase, scndPhaseImages, fileNameOriginal, folderName1):
    ...
    '''
        plot those CC
    '''
    scndPhaseImages, tableImage = extractTableCC(tableCCID, unqElements, labels, nm, img1, phase, scndPhaseImages,
                                                 folderName1)
